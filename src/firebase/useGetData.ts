...
const useGetData = <T extends DataItem>(collectionName: string, condition?: [string, WhereFilterOp, unknown]) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const dataList = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as T));
      setData(dataList);
    } catch (e) {
